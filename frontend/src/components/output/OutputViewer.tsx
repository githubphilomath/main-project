...
import { Download, FileCode, FileText, TestTube, Code, FileType, Globe, Package } from 'lucide-react';
...
export const OutputViewer: React.FC = () => {
  ...
  const allFiles = [
    ...
    ...docArtifacts.map((f) => ({
      ...f,
      file_path: f.doc_type ? `docs/${f.doc_type}.md` : (f as any).file_path || 'docs/unknown.md',
      type: 'doc' as const,
    })),
  ];
  ...
  useEffect(() => {
    if (selectedFileData?.type === 'doc') {
      setViewMode('doc');
    } else if (selectedFileData?.type === 'code' || selectedFileData?.type === 'test') {
      setViewMode('code');
    }
  }, [selectedFile]);
  ...
  return (
    <div className="h-full flex flex-col gap-3">
      ...
      <div className="flex-shrink-0 flex items-center gap-3">
        ...
        {projectStatus.status === 'completed' && currentProject && (
          <Button
            size="sm"
            variant="outline"
            onClick={() => projectsApi.download(currentProject.id)}
      ...
      <div className="flex-1 min-h-0 flex gap-3">
        ...
        <Card className="flex-1 min-w-0 flex flex-col overflow-hidden">
          {/* View mode toggle — always visible when there are files */}
          {allFiles.length > 0 && (
            <CardHeader className="!p-3 flex-shrink-0 flex flex-col gap-2">
              <div className="flex flex-row items-center justify-between gap-2">
                <CardTitle className="!text-sm truncate">
                  {viewMode === 'app' ? 'Full Application Preview' : selectedFileData?.file_path || 'Select a file'}
                </CardTitle>
          ...
          <div className="flex-1 min-h-0 overflow-hidden">
            {viewMode === 'app' && allFiles.length > 0 ? (
              <FullAppPreview projectId={workflowState?.project_id || currentProject?.id || null} />
            ) : selectedFileData ? (
