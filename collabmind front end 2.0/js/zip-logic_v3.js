...
window.downloadProject = async () => {
    ...
    // Tests (New Agent)
    if (window.generatedProject.tests) {
        const testFolder = root.folder("tests");
        Object.keys(window.generatedProject.tests).forEach(file => {
            testFolder.file(file, window.generatedProject.tests[file]);
        });
    }
